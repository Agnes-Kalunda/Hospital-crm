...
const CustomDatePicker = ({ doctorId, selectedDate, handleDateChange }) => {
 ...
 const fetchDoctorAvailability = async (doctorId) => {
   try {
     const res = await axios.get(`/api/doctors/${doctorId}/availabilities/`);
     const dayMap = { 'SUN': 0, 'MON': 1, 'TUE': 2, 'WED': 3, 'THU': 4, 'FRI': 5, 'SAT': 6 };
     const days = res.data.map(avail => dayMap[avail.day_of_week]);
 ...
 return (
   <DatePicker
     ...
     minDate={new Date()}
     dayClassName={highlightAvailableDays}
   ...
   try {
     ...
     if (availability) {
       ...
       if (selectedDoctor) {
         ...
         toast.info(`${doctorName} is available on ${dayName} between ${startTime} and ${endTime}`, {
           position: "top-center"
         });
       }
       
       // Get booked appointments
       const bookedRes = await axios.get(`/api/appointments/?doctor=${doctorId}&date=${date}&status=SCHEDULED`);
       const bookedTimes = bookedRes.data.map(app => {
         const appTime = new Date(app.appointment_datetime);
         return `${appTime.getHours().toString().padStart(2, '0')}:${appTime.getMinutes().toString().padStart(2, '0')}`;
       });
       ...
       while (current <= endTime) {
         ...
         const status = bookedTimes.includes(current) ? 'booked' : 'available';
         
         // If editing, include current time slot
         const isCurrentSlot = isEdit && selectedTime === current;
         
         allSlots.push({
           time: current,
           status: isCurrentSlot ? 'current' : status
         });
                 ...
                 >
                   ...
                   {availableSlots.map(slot => (
                     <option 
                       key={slot.time} 
                       value={slot.time}
                       disabled={slot.status === 'booked' && slot.status !== 'current'}
                       className={
                         slot.status === 'booked' ? 'text-red-400 bg-red-50' : 
                         slot.status === 'current' ? 'text-blue-600 font-medium' : 
                         'text-gray-900'
                       }
                     >
